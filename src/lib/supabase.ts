...
const STORAGE_KEY = 'pizzadon_db_state';

/**
 * Fetch full database state across all available layers.
 * 1. Checks /api/config runtime environment variables to initialize Supabase & Firebase
 * 2. Tries Firebase Firestore (site_state/config)
 * 3. Tries Supabase (pizzadon_site_state)
 * 4. Tries Express Server API (/api/state)
 * 5. Tries localStorage cache
 * 6. Falls back to default seed data
 */
export async function fetchDatabaseState(): Promise<DatabaseState> {
  ...
  // B. Try fetching state from Firebase Firestore
  try {
    ...
    if (db) {
      ...
      if (snap.exists()) {
        ...
        if (fbState && Array.isArray(fbState.items) && fbState.items.length > 0) {
          try { localStorage.setItem(STORAGE_KEY, JSON.stringify(fbState)); } catch {}
          return fbState;
        }
      }
    }
  ...
  // C. Try fetching state from Supabase
  if (supabase) {
    try {
      ...
      if (!error && data && data.state) {
        ...
        if (remoteState.items && Array.isArray(remoteState.items) && remoteState.items.length > 0) {
          try { localStorage.setItem(STORAGE_KEY, JSON.stringify(remoteState)); } catch {}
          return remoteState;
        }
      }
    } catch (err) {
  ...
  try {
    ...
    if (apiRes.ok) {
      ...
      if (serverState && Array.isArray(serverState.items) && serverState.items.length > 0) {
        try { localStorage.setItem(STORAGE_KEY, JSON.stringify(serverState)); } catch {}
        return serverState;
      }
    }
  } catch {}

  // E. Fallback to localStorage cache
  try {
  ...
  // F. Fallback to default initial database state
  return INITIAL_DATABASE_STATE;
}

/**
 * Save updated database state to localStorage, Express server, Supabase, and Firebase.
 */
