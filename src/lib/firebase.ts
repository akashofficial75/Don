...
/**
 * Verify admin password against stored hash in Firebase Firestore (or fallback).
 */
export async function verifyAdminPasswordFirebase(passwordInput: string): Promise<boolean> {
  ...
  if (db) {
    try {
      ...
      if (docSnap.exists()) {
        ...
        // Document doesn't exist yet in Firestore -> initialize with default hashed password
        const defaultHash = await hashPassword('pizzadon2026');
        await setDoc(docRef, {
  ...
  // Check cached password hash from localStorage
  const cachedHash = localStorage.getItem('pizzadon_admin_pwd_hash');
  if (cachedHash) {
    return cachedHash === inputHash;
  }

  // Default initial fallback (hash check)
  const defaultHash = await hashPassword('pizzadon2026');
  return inputHash === defaultHash;
}

/**
 * Change admin password securely and store SHA-256 hash in Firebase Firestore.
 */
export async function changeAdminPasswordFirebase(
  ...
  // 3. Cache locally
  try {
    localStorage.setItem('pizzadon_admin_pwd_hash', newHash);
  } catch {}
