...
export default function AdminPanel({
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Send password change to backend server endpoints first
      let res = await fetch('/.netlify/functions/change-admin-password', {
        method: 'POST',
      ...
      if (res.status === 404) {
        res = await fetch('/api/admin/change-password', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            currentPassword,
      ...
      // 2. Also update Firebase / client local hash cache if Firebase is configured
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setAuthError('');
    try {
      // 1. Check password against Firebase / hashed storage
      ...
      // 2. Try serverless auth function first (Netlify deployment)
      let res = await fetch('/.netlify/functions/verify-admin', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ password: passwordInput })
      });

      // 3. If function route returns 404 (e.g. running on Express backend container), fall back to Express login
      if (res.status === 404) {
